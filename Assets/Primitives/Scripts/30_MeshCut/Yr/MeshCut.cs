...
namespace Yr
{
  public class MeshCut
  {
    ...
    private bool isFillCaps = true;
    private bool useCapMaterial = false;
    private bool flipNormalForMeshCut = false;


    // public Mesh[] Slice(Mesh victimMesh, Vector3 planeNormal, Vector3 planeAnchor, Material capMaterial)
    public Mesh[] Slice(Mesh victimMesh, Vector3 planeNormal, Vector3 planeAnchor, bool isFillCaps, bool useCapMaterial, bool flipNormalForMeshCut)
    {
      this.isFillCaps = isFillCaps;
      this.useCapMaterial = useCapMaterial;
      this.flipNormalForMeshCut = flipNormalForMeshCut;

      this.planeNormal = planeNormal;

      Vector3 inverseNormal = -planeNormal;
      Vector3 inverseAnchor = planeAnchor;
      plane.UpdateParam(inverseNormal, inverseAnchor);

      List<Mesh> sliced = new List<Mesh>();

      Mesh originalMesh = victimMesh;
      ...
      if (this.useCapMaterial) CapCut(originalMesh.subMeshCount);
      else CapCut(originalMesh.subMeshCount - 1);
      ...
      sliced.Clear();
      sliced.Add(leftHandSideMesh);
      sliced.Add(rightHandSideMesh);
      ...
      for (int i = 0; i < newVerticesCache.Count; i += 2)
      {
        if (!capUsedIndices.Contains(i))
        {
          ...
          while (isDone == false)
          {
            ...
            // loop through newVerticesCache to find next chain
          ...
          {
            // if start point & end point are same, rename index in order to make "index" loop
    ...
    {
      // Debug.Log("CALLED - FillCap");
      // Debug.Log($"indices.Count: {indices.Count}");
      // Debug.Log($"newVerticesCache.Count: {newVerticesCache.Count}");
      // Debug.Log($"{string.Join(",", indices)}");

      // compute center. Once we get the temporary center to get the exact center
      Vector3 tempCenter = Vector3.zero;
      foreach (var i in indices)
      {
        tempCenter += newVerticesCache[i];
      }
      tempCenter /= indices.Count;
      // Debug.Log($"tempCenter: {tempCenter}");
      // var exactCenter = tempCenter;


      // We the exact center from Bounds
      var meshForExactCenter = new Mesh();
      var vtxForExactCenter = new List<Vector3>();
      var meshForExactCenterBounds = new Bounds();
      var exactCenter = new Vector3();
      for (int i = 0; i < indices.Count - 1; i++)
      {
        vtxForExactCenter.Add(newVerticesCache[indices[i]]);
        vtxForExactCenter.Add(newVerticesCache[indices[i + 1]]);
        vtxForExactCenter.Add(tempCenter);
      }
      meshForExactCenter.SetVertices(vtxForExactCenter);
      meshForExactCenter.RecalculateBounds();
      meshForExactCenterBounds = meshForExactCenter.bounds;
      exactCenter = meshForExactCenterBounds.center;
      // Debug.Log($"exactCenter: {exactCenter}");

      var distFromPlane = plane.DistFromPlane(exactCenter);
      var displacement2 = -plane.GetNormal() * distFromPlane;
      var targetPoint = exactCenter + displacement2;
      exactCenter = targetPoint;
      ...
      for (int i = 0; i < indices.Count - 1; ++i)
      {
        ...
        // TODO: Refactor to connect triangle each by each, that is consumes two edge and product one edge

        // use displacement to calculate uv
        displacement = newVerticesCache[indices[i]] - exactCenter;
        uv1.x = Vector3.Dot(displacement, left) + 0.5f;
        uv1.y = Vector3.Dot(displacement, up) + 0.5f;
        displacement = newVerticesCache[indices[i + 1]] - exactCenter;
        ...
        triangleCache.vertices[2] = exactCenter;
