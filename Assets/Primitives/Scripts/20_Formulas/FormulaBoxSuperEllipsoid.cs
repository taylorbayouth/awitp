...
{
  public class FormulaBoxSuperEllipsoid : AbstractFormula
  {
    public MyMeshInfo CalculateMyMeshInfo(BoxSuperEllipsoidParameters parameter)
    {
      ...
      for (int j = 0; j < parameter.Segments + 1; j++)
      {
        ...
        for (int i = 0; i < parameter.Segments + 1; i++)
        {
          ...
          // Create Vertex
          float x = parameter.Width / 2.0f * Mathf.Pow(Abs_cosTheta, parameter.N1) * Sign_cosTheta * Mathf.Pow(Abs_cosBete, parameter.N2) * Sign_cosBete;
          float y = parameter.Height / 2.0f * Mathf.Pow(Abs_sinTheta, parameter.N1) * Sign_sinTheta;
          float z = parameter.Depth / 2.0f * Mathf.Pow(Abs_cosTheta, parameter.N1) * Sign_cosTheta * Mathf.Pow(Abs_sinBete, parameter.N2) * Sign_sinBete;
